...
def simple(g):
    ...
    # YOUR CODE HERE
    secondary = g.edges.select("dst", "src")
    edge = secondary.unionAll(g.edges)

    g = GraphFrame(g.vertices, edge)
# Combine old and new edges. Distinctify to eliminate multi-edges
    ...
    # YOUR CODE HERE
    ed = g.edges.filter("src!=dst")
    return GraphFrame(g.vertices, ed)
...
def degreedist(g):
    ...
    # YOUR CODE HERE
    counter = g.inDegrees.selectExpr(
        'id as id', 'inDegree as degree').groupBy('degree').count()

    ns = [int(row['count']) for row in counter.collect()]
    degrees = [int(row['degree']) for row in counter.collect()]
    pyplt.plot(degrees, ns)
    pyplt.ylabel('Count - Y')
    pyplt.xlabel('Degree - X')
    pyplt.savefig(out+".PNG")
    print(powerlaw.Fit(ns).power_law.alpha)
    return counter
...
def readFile(filename, large, sqlContext=sqlContext):
    ...
    # graphframe vertices
    # YOUR CODE HERE
    e = sqlContext.createDataFrame(p, ['src', 'dst'])
    v = edges.selectExpr('src as id').unionAll(
        edges.selectExpr('dst as id')).distinct()
