...
def records_to_graph():
    ...
    for i, j in ratings.items():
        cur = Node(id=temp_id, name=i, type="u")
        nodelist += [cur]
        nodedict[i] = cur
        temp_id += 1

    for _, m in enumerate(movies):
        cur = Node(id=temp_id, name=m, type="m")
        nodelist += [cur]
        nodedict[m] = cur
        temp_id += 1
        ...
        i = 1
        while i < 6:
            cur = Node(id=temp_id, name=m+"_"+str(i), type="r")
            nodelist += [cur]
            temp_id += 1
            i += 1

    for _, dir_ in enumerate(directors):
        cur = Node(id=temp_id, name=dir_, type="d")
        nodelist += [cur]
        nodedict[dir_] = cur
        temp_id += 1

    for _, act in enumerate(actors):
        cur = Node(id=temp_id, name=act, type="a")
        nodelist += [cur]
        nodedict[act] = cur
        temp_id += 1

    for _, g in enumerate(genres):
        cur = Node(id=temp_id, name=g, type="g")
        nodelist += [cur]
        nodedict[g] = cur
        temp_id += 1
    ...
    for k, v in ratings.items():
        # cur_rating = ratings[]
        for cur_mov, cur_rating in v.items():
            # source = k
            destination = cur_mov + "_" + cur_rating
            try:
                user, u_rating = nodedict[k], nodedict[destination]
            except KeyError:
                continue
            user.neighbors += [u_rating]
            u_rating.neighbors += [user]

    for mk, mv in movies.items():
        node = nodedict[mk]
        i = 1
        while i < 6:
            try:
                mov_rating = nodedict[mk + "_" + str(i)]
            except KeyError:
                continue
            mov_rating.neighbors += [node]
            node.neighbors += [mov_rating]
            i += 1

        if mv.director in nodedict:
            dnode = nodedict[mv.director]
            dnode.neighbors += [node]
            node.neighbors += [dnode]

        for _, a in enumerate(mv.actors):
            anode = nodedict[a]
            anode.neighbors += [node]
            node.neighbors += [anode]

        for _, g in enumerate(mv.genres):
            gnode = nodedict[g]
            gnode.neighbors += [node]
            node.neighbors += [gnode]
