...
def make_attr_map(in_file):
    ...
    keys = in_file.__next__().split(",")

    for line in in_file:
        values = map(lambda x: int(x), line.split(","))
        attr_map[int(i)] = list(values)
        i += 1
...
def sac1(graph):
    ...
    for k in range(0, 15):
        ...
        for k in range(0, 15):
            ...
            diff = reduce(lambda x,y: x+y, list(map(lambda x: 1 if x[0] != x[1] else 0, zip(starting_membership, membership))), 0)
        ...
        if previous_communities is None:
            ...
            community_list = list(map(lambda x: [item for sublist in x for item in sublist], community_list))
