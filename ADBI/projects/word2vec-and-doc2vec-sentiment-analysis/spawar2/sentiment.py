        ...
        for w in set(l):
            if w not in stopwords:
                if w in neg:
                    ...
                    neg[w] = 1
    nl, pl = len(train_neg) // 100, len(train_pos) // 100

    f = list()
    for w in pos:
        if pos[w] >= pl and pos[w] // 2 >= neg.get(w, 0):
            f += [w]
    
    for w in neg:
        if neg[w] >= nl and neg[w] // 2 >= pos.get(w, 0):
            f += [w]
        ...
        for l in a:
            for x in b:
                if x in l:
                    ...
                    v += [0]
    train_pos_vec = gen(train_pos, f)
    train_neg_vec = gen(train_neg, f)
    test_pos_vec = gen(test_pos, f)
    test_neg_vec = gen(neg, f)
...
def feature_vecs_DOC(train_pos, train_neg, test_pos, test_neg):
    ...
    def gen_lables(d, l):
        ...
        for i in range(len(d)):
            labs += [TaggedDocument(words=d[i], tags=[l + str(i)])]
        return labs

    labeled_train_pos = gen_lables(train_pos, "TRAIN_POS_")
    labeled_train_neg = gen_lables(train_neg, "TRAIN_NEG_")
    labeled_test_pos = gen_lables(test_pos, "TEST_POS_")
    labeled_test_neg = gen_lables(test_neg, "TEST_NEG_")
    ...
    def gen2(a, b):
        ...
        for i, j in enumerate(a):
            v += [model.docvecs[b + str(i)]]
        return v
    train_pos_vec = gen2(train_pos, "TRAIN_POS_")
    train_neg_vec = gen2(train_neg, "TRAIN_NEG_")
    test_pos_vec = gen2(test_pos, "TEST_POS_")
    test_neg_vec = gen2(test_neg, "TEST_NEG_")
...
def evaluate_model(model, test_pos_vec, test_neg_vec, print_confusion=False):
    ...
    # YOUR CODE HERE
    tp, tn, fp, fn = 0, 0, 0, 0
    pp = model.predict(test_pos_vec)
    np = model.predict(test_neg_vec)
